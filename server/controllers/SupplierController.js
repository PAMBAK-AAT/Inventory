...
const addSupplier = async (req, res) => {
    try {
        // Extract businessContext from the request
        const {name, email, number, address, businessContext} = req.body;
        ...
        const newSupplier = new SupplierModel({ name, email, number, address, businessContext });
        await newSupplier.save();
        return res.status(201).json({ success: true, message: "Supplier added successfully" });
    } catch (error) {
        console.error("Error adding Supplier:", error);
...
const getSuppliers = async (req, res) => {
    try{
        const suppliers = await SupplierModel.find().sort({ createdAt: -1 }); // Sort newest first
        return res.status(200).json({success: true, suppliers});
    }catch (error){
        console.error("Error fetching suppliers:", error);
        return res.status(500).json({success: false, message: "Server error in getting suppliers"});
...
const updateSupplier = async (req, res) => {
    try {
        const { id } = req.params;
        // Extract businessContext for updates
        const { name, email, number, address, businessContext } = req.body;

        const updatedSupplier = await SupplierModel.findByIdAndUpdate(
            id,
            { name, email, number, address, businessContext },
            { new: true } 
        );
